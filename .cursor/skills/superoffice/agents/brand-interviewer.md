...
(a) **템플릿 파일 분석 (회사 자체 양식이 있을 때 1순위)** - 사용자가 `.pptx`/`.docx`/`.xlsx`를 줬으면 **직접 뽑을 수 있는 것만** 추출한다. 못 뽑는 항목만 (b)로 넘긴다 (위조 금지 - 추측한 색을 "추출됨"이라 쓰지 않는다). 추출 후 출처와 불확실성을 conductor에게 보고한다. 불명확하거나 기존 지침과 충돌하는 값만 확인받고, 명확하게 제공·승인된 값은 반복 승인 없이 저장한다. 추출한 레이아웃·섹션 구조(제목 스타일·표 모양·슬라이드 순서)는 `format_skeleton`으로 함께 기록해 `reference/examples.md`의 형식 예시로 넘긴다(회사 양식 = 브랜드 + 형식 이중 소스). 양식이 없으면 이 경로를 건너뛰고 (b)로 간다.
