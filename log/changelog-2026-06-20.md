...
- **한글 띄어쓰기 진단**: korean-gate는 의도적 고신뢰 floor라 spacing 규칙이 4개(것/수/척/체 의존명사)뿐 - 완전한 띄어쓰기는 문맥 의존적이라 형태소 분석기 없이는 오탐 위험, LLM 생성 + doc-critic 판단이 담당. korean-rules.json에 안전한 의존명사 규칙 4종 추가(-ㄹ 것/만큼/줄/리). 실측: 오류 4종 정확 발동, 견본·reference 15파일 오탐 0. 랜딩 페이지의 게이트 데모 줄(일부러 "할수있다" 오류 예시)은 `edu-ok` HTML 주석으로 억제.
