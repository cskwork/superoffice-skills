...
- **Verification, not eyeballing.** A deterministic gate (`templates/office-gate.sh`) scans the built text evidence - Korean spelling/spacing, factual integrity (sourced), emoji/PII, color contrast. The builder never self-approves; an independent critic runs the gate. For binary outputs an optional dependency, OfficeCLI (`brew`/`scoop`/`npm`, Apache-2.0), renders a screenshot for visual review and flags text overflow (`view issues`) / OpenXML validity - skipped and disclosed when absent, never faked.
